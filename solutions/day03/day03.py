...
nums: list[tuple[int, int, int]] = list(
    map(
        lambda m: (int(m.group(0)), m.start(), m.end()),
        re.finditer(r"(\d+)", schematic),
    )
)
...
def part1():
    symbols = [
        i for i, c in enumerate(schematic) if c in string.punctuation and c != "."
    ]
...
def part2():
    ...
    for x in gears:
        matches = {
            num for num, start, end in nums if any(start <= x + d < end for d in dx)
        }
